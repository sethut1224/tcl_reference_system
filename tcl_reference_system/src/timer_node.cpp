...
namespace tcl_reference_system
{
    ...
    : Node("timer_node", options)
    {
        ...
        {   
            ...
            {
                sub_map_[topic] = this->create_subscription<DynamicMessage>(topic, qos, std::bind(&TimerNode::normal_topic_callback, this, std::placeholders::_1));
            }
            else
            {
                sub_map_[topic] = this->create_subscription<DynamicMessage>(topic, qos, std::bind(&TimerNode::blocking_topic_callback, this, std::placeholders::_1));
            }
        });
        ...
        auto period = this->get_node_timing_coordination_interface()->get_period_ns();
    ...
    void
    TimerNode::normal_topic_callback(const DynamicMessage::SharedPtr msg)
    {
        (void)msg;
    }

    void
    TimerNode::blocking_topic_callback(const DynamicMessage::SharedPtr msg)
    {
        int sum = 0;
